...
description: Orchestrate a complete Tic-Tac-Toe game between two AI agents, managing the game flow, enforcing rules, and maintaining the game state in a markdown file for human readability.
...
7. **Ask the commentator** to comment on the move and the current board state by invoking `#tic-tac-toe-plays/comment-move`
...
5. **Ask the commentator** to provide a final analysis of the game, highlighting the most exciting moments and the overall flow of the match by invoking `#tic-tac-toe-plays/summarize-game`
...
NEVER DISPLAY THE BOARD STATE BEFORE THE MOVE.
```
...
Board:
{display the board transition}

Commentary:
{commentator's analysis}

{If game over: display the result and display the summary from commentator}
```
